...
function getSumOfDigits(num) {
  const absoluteNum = Math.abs(Math.floor(num));
  const digitsArray = absoluteNum.toString().split('');
  const sum = digitsArray.reduce((acc, digit) => acc + parseInt(digit, 10), 0);
  return sum;
}
...
function getNumberValue(number) {
  ...
  throw new Error('Argument must be a Number object or a primitive number.');
}
...
function getIntegerOnString(str, base) {
  ...
  return Number.isInteger(parsedInteger) ? parsedInteger : NaN;
}
...
function getRandomInteger(min, max) {
  if (!Number.isInteger(min) || !Number.isInteger(max)) {
    throw new Error('Input should be integers.');
  }
  if (min > max) {
